...
if __name__ == '__main__':
    ...
    # We must patch tokenizer.json directly instead of creating vocab.json/merges.txt
    tokenizer_json_path = os.path.join(output_dir, 'tokenizer.json')
    if os.path.exists(tokenizer_json_path):
        with open(tokenizer_json_path, 'r', encoding='utf-8') as f:
            full_data = json.load(f)
            
        full_data["model"]["vocab"] = vocab
        full_data["model"]["merges"] = merges
        
        with open(tokenizer_json_path, 'w', encoding='utf-8') as f:
            json.dump(full_data, f, ensure_ascii=False, indent=2)
    else:
        # Fallback if tokenizer.json is missing for some reason
    ...
    # We skip re-loading and re-saving to avoid HF slow tokenizer conversion issues
