...
def predict_vec(input_ids, model, mlp_head, mode="mlp"):
    with torch.no_grad():
        ...
        if mode == "mlp":
            ...
            pred_embeds = hidden_states[:, 0, :, :].mean(dim=1).to(model.device)[0] # Taking layer 0 (embeddings) and averaging over sequence length
        else:
    ...
    for i in tqdm(range(copy_limit, new_vocab_size)):
        ...
        if embed_tokens_ids is None:
            ...
            if token_decoded == ' историческая':
                print(tasks[-1])
                print(predict_vec(embed_tokens_ids, model, mlp_head, mode=mode))
